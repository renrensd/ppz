/* Automatically generated by gen_abi from /home/lijie/work/codes/ef01_ac/ac_code/conf/abi.xml */
...
#define ABI_FLOW_ID 13
#define ABI_RADAR_24_ID 14

/* Array and linked list structure */
#define ABI_MESSAGE_NB 15

ABI_EXTERN abi_event* abi_queues[ABI_MESSAGE_NB];

/* Callbacks */
typedef void (*abi_callbackBARO_ABS)(uint8_t sender_id, uint32_t stamp, float pressure, float temperature);
...
typedef void (*abi_callbackFLOW)(uint8_t sender_id, struct Px4_flow_Data * flow_data);
typedef void (*abi_callbackRADAR_24)(uint8_t sender_id, float distance);
...
static inline void AbiSendMsgBARO_ABS(uint8_t sender_id, uint32_t stamp, float pressure, float temperature) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_BARO_ABS_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      ...
      cb(sender_id, stamp, pressure, temperature);
    }
  }
}
...
static inline void AbiBindMsgFLOW(uint8_t sender_id, abi_event * ev, abi_callbackFLOW cb) {
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_FLOW_ID],ev);
}

static inline void AbiSendMsgFLOW(uint8_t sender_id, struct Px4_flow_Data * flow_data) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_FLOW_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackFLOW cb = (abi_callbackFLOW)(e->cb);
      cb(sender_id, flow_data);
    }
  }
}

static inline void AbiBindMsgRADAR_24(uint8_t sender_id, abi_event * ev, abi_callbackRADAR_24 cb) {
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_RADAR_24_ID],ev);
}

static inline void AbiSendMsgRADAR_24(uint8_t sender_id, float distance) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_RADAR_24_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackRADAR_24 cb = (abi_callbackRADAR_24)(e->cb);
      cb(sender_id, distance);
    }
  }
}
