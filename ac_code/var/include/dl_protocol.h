...
#define DOWNLINK_SEND_ACINFO(_trans, _dev, course, utm_east, utm_north, alt, itow, speed, climb, ac_id, wp_id) pprz_msg_send_ACINFO(&((_trans).trans_tx), &((_dev).device), AC_ID, course, utm_east, utm_north, alt, itow, speed, climb, ac_id, wp_id)
static inline void pprz_msg_send_ACINFO(struct transport_tx *trans, struct link_device *dev, uint8_t ac_id, int16_t *_course, int32_t *_utm_east, int32_t *_utm_north, int32_t *_alt, uint32_t *_itow, uint16_t *_speed, int16_t *_climb, uint8_t *_ac_id, uint8_t *_wp_id) {
	if (trans->check_available_space(trans->impl, dev, trans->size_of(trans->impl, 0+2+4+4+4+4+2+2+1+1 +2 /* msg header overhead */))) {
	  trans->count_bytes(trans->impl, dev, trans->size_of(trans->impl, 0+2+4+4+4+4+2+2+1+1 +2 /* msg header overhead */));
	  trans->start_message(trans->impl, dev, 0+2+4+4+4+4+2+2+1+1 +2 /* msg header overhead */);
...
#define DL_ACINFO_wp_id(_payload) ((uint8_t)(*((uint8_t*)_payload+25)))
