...
let () =
  ...
  try
    ...
    if (class_name = "telemetry" || class_name ="ac") then begin (** FIXME *)
      Printf.fprintf h "#if DOWNLINK\n"
    end;
    Gen_onboard.print_downlink_macros h class_name messages;
    
    if (class_name = "telemetry" || class_name ="ac") then begin
    ...
    let check_alignment = (class_name <> "telemetry" && class_name <>"ac" && class_name <>"rc" && class_name <>"gcs" && class_name <>"lc" && class_name <>"datalink" && class_name <>"intermcu") in
