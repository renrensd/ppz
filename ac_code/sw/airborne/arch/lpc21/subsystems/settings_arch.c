																		...
																		uint32_t chksum)
{
	...
	/* erase */
	if ((ret = pflash_erase_page(flash)))
	{
		return ret;
	}
	...
	for (i = 0; i < size; i += BOUND)
	{
		...
		if ((ret = pflash_program_array(flash, flash->addr + i, ptr)))
		{
			return ret;
		}
	}
	...
	for (i = 0; i < size; i++)
	{
		if ((*(uint8_t *)(flash->addr + i)) != (*(uint8_t *)(src + i)))
		{
			return -2;
		}
	}
	if (*(uint32_t *)(flash->addr + flash->page_size - FSIZ) != size)
	{
		return -3;
	}
	if (*(uint32_t *)(flash->addr + flash->page_size - FCHK) != chksum)
	{
		return -4;
	}
...
int32_t persistent_write(void *ptr, uint32_t size)
{
	...
	if (flash_detect(&flash_info))
	{
		return -1;
	}
	if ((size > flash_info.page_size - FSIZ) || (size == 0))
	{
		return -2;
	}
...
int32_t persistent_read(void *ptr, uint32_t size)
{
	...
	/* check parameters */
	if (flash_detect(&flash))
	{
		return -1;
	}
	if ((size > flash.page_size - FSIZ) || (size == 0))
	{
		return -2;
	}

	/* check consistency */
	if (size != *(uint32_t *)(flash.addr + flash.page_size - FSIZ))
	{
		return -3;
	}
