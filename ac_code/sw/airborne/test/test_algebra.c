...
float test_INT32_QUAT_OF_RMAT(struct FloatEulers *eul_f, bool_t display)
{
	...
	if (display)
	{
		DISPLAY_FLOAT_RMAT("rmat float", rmat_f);
	}
	...
	if (display)
	{
		DISPLAY_INT32_RMAT("rmat int", rmat_i);
	}
	if (display)
	{
		DISPLAY_INT32_RMAT_AS_FLOAT("rmat int", rmat_i);
	}
	...
	if (display)
	{
		DISPLAY_INT32_QUAT("qi", qi);
	}
	if (display)
	{
		DISPLAY_INT32_QUAT_2("qi", qi);
	}
