...
#define HMC58XX_DEFAULT_DO 0x7 // Data Output Rate (6 -> 50Hz with HMC5843, 75Hz with HMC5883, 7->220Hz for HMC5983)
#endif
...
#ifndef HMC58XX_DEFAULT_TS
#define HMC58XX_DEFAULT_TS 0x1 // Continious measurement mode
#endif
...
static void hmc58xx_send_config(struct Hmc58xx *hmc)
{
  switch (hmc->init_status) {
    case HMC_CONF_CRA:
      hmc58xx_i2c_tx_reg(hmc, HMC58XX_REG_CFGA,(hmc->config.ts << 7) | (hmc->config.rate << 2) | (hmc->config.meas));
      hmc->init_status++;
      break;
    case HMC_CONF_CRB:
  ...
  if (hmc->initialized) {
    if (hmc->i2c_trans.status == I2CTransFailed) {
      ...
      /* HMC5883 has xzy order of axes in returned data */
        ...
        hmc->data.vect.x = Int16FromBuf(hmc->i2c_trans.buf, 0);//5983:x,z,y
