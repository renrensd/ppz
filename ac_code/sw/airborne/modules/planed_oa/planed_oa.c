...
static void get_obstacle_info(void);
static void planed_oa_run(void);
static void planed_oa_geometry_int(void);

/*enum _e_segment_relation rel_12;
enum _e_segment_relation rel_23;
enum _e_segment_relation rel_34;
enum _e_segment_relation rel_41;*/
...
void planed_oa_data_init(void)
{
	...
	{
	   planed_oa.o[i].obstacle_flag = FALSE;
	   planed_oa.o[i].pos.x = 0.0f;
	   planed_oa.o[i].pos.y = 0.0f;
	   planed_oa.o[i].raduis = 0.0f;
	}
...
static void planed_oa_geometry_int(void)
{
	area_boundary_array_num = spray_boundary_array_num + 1;

		polygon_init(&poly, spray_boundary_array, spray_boundary_array_num);
		polygon_init(&area, area_boundary_array, area_boundary_array_num);

		for (int i = 0; i < obstacles_num; ++i)
		{
			polygon_init(&(obstacles[i]), obstacles_vertices[i], NB_MAX_OBSTACLE_BORDER);
		}

		int area_generate_error = generate_valid_area(&area, &poly, &land_point);
 ...
 */
float vector_angle_calc(struct FloatVect2 *vect_a, struct FloatVect2 *vect_b)
{
 ...
 */
void get_project_point(struct FloatVect2 *proj_point, struct FloatVect2 *start_wp, struct FloatVect2 *end_wp, struct FloatVect2 *point)
{
	float angle, slope;
...
/*
 * Get eliminated min value of dist[]
 */
uint8_t min_dist_eliminate_select(float *dist, uint8_t n, bool_t *flag)
{
	uint8_t min=0;
 ...
 */
bool_t get_initial_start_end_oa_wp( struct FloatVect2 *init_start_oa_wp, struct FloatVect2 *init_end_oa_wp, struct _s_planed_obstacle *obstacle, uint8_t o_num, struct FloatVect2 *start_wp, struct FloatVect2 *end_wp )
{
    if( o_num <= 0 )
    {
	...
	for( int i=0; i<o_num; i++ )
	{
		if( obstacle[i].obstacle_flag )
		{
			...
			{
			    get_project_point( &(project_point[i*OA_OBSTACLE_CORNER_NUM+j]), start_wp, end_wp, &(obstacle[i].v[j]) );
				...
				if( (proj_start_dist[i*OA_OBSTACLE_CORNER_NUM+j] > start_end_dist) || (proj_end_dist[i*OA_OBSTACLE_CORNER_NUM+j] > start_end_dist) )
				{
 ...
 */
void get_s1_s4_wp( struct FloatVect2 *start_oa_wp, struct FloatVect2 *end_oa_wp, struct FloatVect2 *init_start_oa_wp, struct FloatVect2 *init_end_oa_wp, struct FloatVect2 *start_wp, struct FloatVect2 *end_wp )
{   
 ...
 */
static bool_t rectangle_obstacle_on_nav_route( struct _s_planed_obstacle *obstacle, uint8_t o_num, struct FloatVect2 *start_wp, struct FloatVect2 *end_wp )
{
    for(int i=0; i<o_num; i++)
    {
    	...
    	enum _e_segment_relation rel_12 = get_2_segments_strict_relation( start_wp, end_wp, &(obstacle[i].v[0]), &(obstacle[i].v[1]) );
		enum _e_segment_relation rel_23 = get_2_segments_strict_relation( start_wp, end_wp, &(obstacle[i].v[1]), &(obstacle[i].v[2]) );
		enum _e_segment_relation rel_34 = get_2_segments_strict_relation( start_wp, end_wp, &(obstacle[i].v[2]), &(obstacle[i].v[3]) );
		enum _e_segment_relation rel_41 = get_2_segments_strict_relation( start_wp, end_wp, &(obstacle[i].v[3]), &(obstacle[i].v[0]) );
		enum _e_segment_relation rel_13 = get_2_segments_strict_relation( start_wp, end_wp, &(obstacle[i].v[0]), &(obstacle[i].v[2]) );
		enum _e_segment_relation rel_24 = get_2_segments_strict_relation( start_wp, end_wp, &(obstacle[i].v[1]), &(obstacle[i].v[3]) );
 ...
 * return TRUE ;
 */
static bool_t rectangle_obstacle_on_oa_route( struct _s_planed_obstacle *obstacle, uint8_t o_num, struct FloatVect2 *start_wp, struct FloatVect2 *end_wp )
{
...
void get_oa_search_direction(void)
{
    ...
    for( int i=0; i<oa_data.obstacles_num; i++ )
	{   
		VECT2_DIFF( planed_oa.vect_from_to_o, planed_oa.o[i].pos, planed_oa.from_wp);
		...
		get_project_point( &project_point, &planed_oa.from_wp, &planed_oa.next_wp, &(planed_oa.o[i].pos) );
		v_dist[i] = points_length_calc( &project_point, &(planed_oa.o[i].pos) );
			
		if( obstacle_angle < 0.0f )
		{
		    if( v_dist[i] >= planed_oa.o[i].raduis )
		    {
                left_vert_dist_sum = left_vert_dist_sum + 2.0f * planed_oa.o[i].raduis;
		    }
			else
		    {
		        left_vert_dist_sum = left_vert_dist_sum + planed_oa.o[i].raduis + v_dist[i];
                right_vert_dist_sum = right_vert_dist_sum + (planed_oa.o[i].raduis - v_dist[i]);
		    }
		}
		else
		{
		    ...
		    if( v_dist[i] >= planed_oa.o[i].raduis )
		    {
                right_vert_dist_sum = right_vert_dist_sum + 2.0f * planed_oa.o[i].raduis;
			}
			else
		    {
		        right_vert_dist_sum = right_vert_dist_sum + planed_oa.o[i].raduis + v_dist[i];
...
void oa_wp_generation(void)
{
	...
	if( direction_i < 2 )
	{
		...
		{
			get_s1_s4_wp( &planed_oa.oa_wp.s1, &planed_oa.oa_wp.s4, &planed_oa.init_start_oa_wp, &planed_oa.init_end_oa_wp, &planed_oa.from_wp, &planed_oa.next_wp );

			if( vertical_i <= vertical_step_num )
			{
				vertical_search_dist = vertical_i * VERTICAL_STEP_LENGTH;

				planed_oa.oa_wp.s2.x = planed_oa.oa_wp.s1.x - vertical_search_dist * Sign(vert_search_direction) * cosf(planed_oa.nav_north_angle * ANGLE_RAD);
				planed_oa.oa_wp.s2.y = planed_oa.oa_wp.s1.y + vertical_search_dist * Sign(vert_search_direction) * sinf(planed_oa.nav_north_angle * ANGLE_RAD);

				planed_oa.oa_wp.s3.x = planed_oa.oa_wp.s4.x - vertical_search_dist * Sign(vert_search_direction) * cosf(planed_oa.nav_north_angle * ANGLE_RAD);
				planed_oa.oa_wp.s3.y = planed_oa.oa_wp.s4.y + vertical_search_dist * Sign(vert_search_direction) * sinf(planed_oa.nav_north_angle * ANGLE_RAD);
				...
				if( oa_line_i < 3 )
				{
					if(oa_line_i == 0)
					{
						oa_line_in_polygon_flag = is_line_in_polygon( &planed_oa.oa_wp.s1, &planed_oa.oa_wp.s2, &area );

						if( !rectangle_obstacle_on_oa_route( planed_oa.o, obstacles_num, &planed_oa.oa_wp.s1, &planed_oa.oa_wp.s2 ) && oa_line_in_polygon_flag )
						{
							oa_line_i++;
						}
						else
					...
					{
						oa_line_in_polygon_flag = is_line_in_polygon( &planed_oa.oa_wp.s2, &planed_oa.oa_wp.s3, &area );

						bool_t s2_s3_in_poly_flag = TRUE;

						for( int i=0; i<obstacles_num; i++ )
						{
							if( is_point_in_polygon( &planed_oa.oa_wp.s2, &(obstacles[i]) ) || is_point_in_polygon( &planed_oa.oa_wp.s3, &(obstacles[i]) ) )
							{
						...
						if( !rectangle_obstacle_on_oa_route( planed_oa.o, obstacles_num, &planed_oa.oa_wp.s2, &planed_oa.oa_wp.s3 ) && oa_line_in_polygon_flag && s2_s3_in_poly_flag )
						{
							oa_line_i++;
						}
						else
					...
					{
						oa_line_in_polygon_flag = is_line_in_polygon( &planed_oa.oa_wp.s3, &planed_oa.oa_wp.s4, &area );

						if( !rectangle_obstacle_on_oa_route( planed_oa.o, obstacles_num, &planed_oa.oa_wp.s3, &planed_oa.oa_wp.s4 ) && oa_line_in_polygon_flag )
						{
							waypoint_set_enu(WP_S1, &planed_oa.oa_wp.s1);
							waypoint_set_enu(WP_S2, &planed_oa.oa_wp.s2);
							waypoint_set_enu(WP_S3, &planed_oa.oa_wp.s3);
							waypoint_set_enu(WP_S4, &planed_oa.oa_wp.s4);
...
{	
	for(int i=0; i<obstacles_num; i++)
	{
	    planed_oa.o[i].pos.x = ( obstacles[i].v[0].x + obstacles[i].v[2].x ) * 0.5f;
		planed_oa.o[i].pos.y = ( obstacles[i].v[0].y + obstacles[i].v[2].y ) * 0.5f;
		
		planed_oa.o[i].raduis = points_length_calc( &(planed_oa.o[i].pos), &(obstacles[i].v[0]) );
		
		VECT2_COPY( planed_oa.o[i].v[0], obstacles[i].v[0]);
		VECT2_COPY( planed_oa.o[i].v[1], obstacles[i].v[1]);
		VECT2_COPY( planed_oa.o[i].v[2], obstacles[i].v[2]);
		VECT2_COPY( planed_oa.o[i].v[3], obstacles[i].v[3]);
	}
}
 ...
 */
static bool_t find_shortest_avaliable_spray_boundary( struct FloatVect2 *spray_boundary_array, uint8_t spray_boundary_array_num, struct FloatVect2 *insert_oa_wp, struct FloatVect2 *start_wp, struct FloatVect2 *end_wp )
{
    float p2p_dist[OA_MAX_BOUNDARY_VERTICES_NUM];
	
	for(int i=0; i<spray_boundary_array_num; i++)
	{
		if(   is_line_in_polygon( start_wp, &(spray_boundary_array[i]), &area )
		   && is_line_in_polygon( &(spray_boundary_array[i]), end_wp, &area )
		   && spray_boundary_vaild_flag[i] )
		{
			float start_wp_to_spray_boundary_dist = points_length_calc( &spray_boundary_array[i], start_wp );
			float end_wp_to_spray_boundary_dist = points_length_calc( &spray_boundary_array[i], end_wp );
            p2p_dist[i] = start_wp_to_spray_boundary_dist + end_wp_to_spray_boundary_dist;
		}
		else
		{
            p2p_dist[i] = MAX_SELECT_LENGTH;
		}
	}

	int8_t k = min_dist_select(p2p_dist, spray_boundary_array_num);
...
static void planed_oa_run(void)
{
	...
	if( oa_wp_search_state == not_search )
	{
		...
		if( !is_line_in_polygon( &planed_oa.pre_from_wp, &planed_oa.pre_next_wp, &area ) && (insert_state == no_insert_wp) )
		{
			if( find_shortest_avaliable_spray_boundary( spray_boundary_array, spray_boundary_array_num, &planed_oa.insert_oa_wp, &planed_oa.pre_from_wp, &planed_oa.pre_next_wp ) )
			{
			...
			else
			{
				...
				//close for debug:
				get_oa_from_next_wp();
				waypoint_set_enu(WP_From, &planed_oa.from_wp);
				waypoint_set_enu(WP_Next, &planed_oa.next_wp);
				return;
			}
		}
		...
		/* necessity of oa */
		planed_oa.oa_necessity_flag = rectangle_obstacle_on_nav_route( planed_oa.o, obstacles_num, &planed_oa.from_wp, &planed_oa.next_wp );

		if( planed_oa.oa_necessity_flag )
		{
		    /* get initial start end oa wp to calc s1 and s4 */
		    get_initial_start_end_oa_wp( &planed_oa.init_start_oa_wp, &planed_oa.init_end_oa_wp, planed_oa.o, obstacles_num, &planed_oa.from_wp, &planed_oa.next_wp );
		}

		waypoint_set_enu(WP_T1, &planed_oa.init_start_oa_wp);
		waypoint_set_enu(WP_T4, &planed_oa.init_end_oa_wp);

		waypoint_set_enu(WP_T0, &planed_oa.from_wp);
		waypoint_set_enu(WP_T5, &planed_oa.next_wp);
	}
...
void get_vaild_spray_edge(void)
{
    ...
    for( uint8_t i = 0; i < spray_boundary_array_num; i++ )
    {
    	for( uint8_t j = 0; j < obstacles_num; j++ )
    	{
            if( is_point_in_polygon( &(spray_boundary_array[i]), &(obstacles[j]) ) )
            {
...
{
    if( planed_oa.test_on && ( FLIGHT_LINE == from_wp.action || planed_oa.back_home_ready || TERMINATION == from_wp.action ) )
	{
		...
		else
		{
			...
			else if( insert_state == no_insert_wp && oa_wp_search_state > 0 )
			{
...
static void send_point_to_pprz(void)
{
	for (int i = 0; i < OA_MAX_BOUNDARY_VERTICES_NUM; ++i)
	{
		struct EnuCoor_f c;
		c.z = 0;
		if (i < oa_data.spray_boundary_vertices_num)
		{
			VECT2_COPY(c, oa_data.spray_boundary_vertices_array[i]);
			waypoint_set_enu(WP_V0 + i, &c);
		}
		else
		{
			VECT2_COPY(c, oa_data.spray_boundary_vertices_array[oa_data.spray_boundary_vertices_num - 1]);
			waypoint_set_enu(WP_V0 + i, &c);
		}
	}
	...
	for (i = 0; i < OA_MAX_OBSTACLES_NUM; ++i)
	{
		...
		if (i < oa_data.obstacles_num)
		{
			for (j = 0; j < 4; j++)
			{
				VECT2_COPY(c, oa_data.obstacles_vertices_array[i][j]);
				waypoint_set_enu(WP_O11 + (4 * i + j), &c);
			}
		}
		else
		{
	...
	for (int i = 0; i <= OA_MAX_BOUNDARY_VERTICES_NUM; ++i)
	{
		...
		if (i < area.n)
		{
			VECT2_COPY(c, area_boundary_array[i]);
			waypoint_set_enu(WP_A0 + i, &c);
		}
		else
		{
			VECT2_COPY(c, area_boundary_array[area.n - 1]);
			waypoint_set_enu(WP_A0 + i, &c);
		}
	}
}
...
void planed_oa_prepare(void)
{
  ...
  planed_oa_geometry_int();
	get_obstacle_info();
	get_vaild_spray_edge();
	send_point_to_pprz();
...
void planed_oa_periodic_run(void)
{
	...
	if( planed_oa.test_on && oa_wp_search_state < 2 )
	{
		...
		if( (flight_state == cruising) && (guidance_h.mode == GUIDANCE_H_MODE_NAV) && (planed_oa.planed_oa_ready) )
		{
			...
			{
				if( ( !planed_oa.sp_state ) && ( planed_oa.wp_move_done_flag ) && ( oa_wp_search_state == not_search || oa_wp_search_state == on_search ) )
				{
					//run_time[0] = usec_of_sys_time_ticks(sys_time.nb_tick);
					planed_oa_run();
					...
					for(int i = 0; i < OA_MAX_OBSTACLES_NUM; i++)
					{
                         o_flag[i] = planed_oa.o[i].obstacle_flag;
					}
				}
			...
			waypoint_set_enu(WP_From, &planed_oa.from_wp);
			waypoint_set_enu(WP_Next, &planed_oa.next_wp);
			waypoint_set_enu(WP_HOME, &land_point);
		}
