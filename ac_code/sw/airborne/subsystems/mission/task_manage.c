...
//	nb_pending_reland = 0;	//todo:for debug
...
/*	case GCS_CMD_RELAND:	//todo:for debug
...
/*
	if( dl_land_info.land_type_length >= (1+home_offset) )
	{
		...
		{
			if(*(dl_land_info.land_type+i+home_offset) == VERTIPAD_LAND_POINT)  
			{
				response = parse_task_vertipad_land(dl_land_info, home_offset);
