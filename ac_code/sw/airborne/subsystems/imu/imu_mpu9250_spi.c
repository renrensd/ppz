...
#include "imu_mpu9250_spi.h"
#ifdef CALIBRATION_OPTION
#include "calibration.h"
#endif

/* SPI defaults set in subsystem makefile, can be configured from airframe file */
PRINT_CONFIG_VAR(IMU_MPU9250_SPI_SLAVE_IDX)PRINT_CONFIG_VAR(IMU_MPU9250_SPI_DEV)
...
#endif
#endif
PRINT_CONFIG_VAR(IMU_MPU9250_SMPLRT_DIV)PRINT_CONFIG_VAR(IMU_MPU9250_GYRO_LOWPASS_FILTER)PRINT_CONFIG_VAR(
	...
	if (imu_mpu9250.selftest.state != FINISHED)  //no send data from abi
	{
		if (imu_mpu9250.selftest.state == UNTESTED)
		{
			if (((accel.x || accel.y || accel.z) && (rates.p || rates.q || rates.r)))  //request data is valid
			{
				...
				{
					VECT3_SDIV(imu_mpu9250.selftest.static_accel, imu_mpu9250.selftest.static_accel,
							imu_mpu9250.selftest.static_counter);
					RATES_SDIV(imu_mpu9250.selftest.static_gyro, imu_mpu9250.selftest.static_gyro,
							imu_mpu9250.selftest.static_counter);
		...
		else if (imu_mpu9250.selftest.state == CONFIG_TEST)
		{
			...
			&& abs(accel.y) > ACCEL_REPONSE_MIN && abs(rates.p) > GYRO_RESPONSE_MIN && abs(rates.r) > GYRO_RESPONSE_MIN)
			{
				...
				{
					VECT3_SDIV(imu_mpu9250.selftest.test_accel, imu_mpu9250.selftest.test_accel,
							imu_mpu9250.selftest.test_counter);
					RATES_SDIV(imu_mpu9250.selftest.test_gyro, imu_mpu9250.selftest.test_gyro, imu_mpu9250.selftest.test_counter);
					/*caculate response data*/
					VECT3_DIFF(imu_mpu9250.selftest.response_accel, imu_mpu9250.selftest.test_accel,
							imu_mpu9250.selftest.static_accel);
					RATES_DIFF(imu_mpu9250.selftest.response_gyro, imu_mpu9250.selftest.test_gyro,
							imu_mpu9250.selftest.static_gyro);
					if ( INTERVAL_ABS_VALID(imu_mpu9250.selftest.response_accel.x, ACCEL_REPONSE_MIN,
							ACCEL_RESPONSE_MAX) && INTERVAL_ABS_VALID(imu_mpu9250.selftest.response_accel.y, ACCEL_REPONSE_MIN, ACCEL_RESPONSE_MAX)
		...
		else if (imu_mpu9250.selftest.state == RECOVER_CHECK)
		{
			...
			&& abs(accel.y) < (ACCEL_REPONSE_MIN / 5) && abs(rates.p) < (GYRO_RESPONSE_MIN / 10)
...
#if IMU_MPU9250_READ_MAG
	if (!bit_is_set(imu_mpu9250.mpu.data_ext[6], 3))
	{ //mag valid just HOFL == 0
...
bool_t imu_mpu9250_configure_mag_slave(Mpu9250ConfigSet mpu_set, void *mpu)
{
	...
	mpu_set_and_wait(mpu_set, mpu, MPU9250_REG_I2C_SLV0_CTRL, (1 << 7) |    // Slave 0 enable
