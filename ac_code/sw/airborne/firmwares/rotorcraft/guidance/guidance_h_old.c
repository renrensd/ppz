...
static void guidance_h_traj_run(bool_t in_flight)
{
  static uint8_t flag_integrator=1;
  ...
  if (in_flight) {
    ...
    guidance_h_trim_att_integrator.x += flag_integrator*(guidance_h_igain * pd_x);
    guidance_h_trim_att_integrator.y += flag_integrator*(guidance_h_igain * pd_y);
    /* saturate it  */
  ...
  //take off and heading change,attitude angle is 3deg limited
  if( (agl_dist_value_filtered<0.4)||(abs(stab_att_ref_rate.r)>=BFP_OF_REAL(0.37, INT32_RATE_FRAC))  ) 
  { 
  	VECT2_STRIM(guidance_h_cmd_earth, -total_limit_bank, total_limit_bank); 
	flag_integrator=0;
  } 
  else flag_integrator=1;
